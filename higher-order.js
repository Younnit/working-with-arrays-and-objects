...
//Code Here
let evenNumbers = mixedNumbers.filter((number, index, array)=> number % 2 === 0)
...
//Code Here
let postTaxPrices = prices.map((number, index, array)=>{
  return number * 1.07
});
...
//Code Here
let myStrongest = monstersInYourPocket.filter((monster, index, array)=>{
  if (monster.CP > 200){
    return monster
  }
})
...
let orderTotals = orders.map((number)=>{
  return (number.price * number.tax) + number.price
})
...
let bobsTotal = purchases.reduce((total, name)=>{
  if(name.owner === 'Bob'){
  total += name.price
  }
  return total
